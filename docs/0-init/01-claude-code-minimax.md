...
*(Nếu bạn từng thêm dòng `export` trong file `~/.bashrc`, `~/.zshrc` hoặc Environment Variables của Windows, hãy xóa các dòng đó)*.
...
> - `ANTHROPIC_AUTH_TOKEN`: Thay `<MINIMAX_API_KEY>` bằng mã API Key lấy từ [MiniMax Platform](https://platform.minimax.io/user-center/payment/token-plan).
